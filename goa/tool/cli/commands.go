...
func (cmd *CreateOrJoinClusterCommand) Run(c *client.Client, args []string) error {
	...
	resp, err := c.CreateOrJoinCluster(ctx, path, &payload)
	if err != nil {
...
func (cmd *StatusClusterCommand) Run(c *client.Client, args []string) error {
	...
	if len(args) > 0 {
		...
		path = fmt.Sprintf("/cluster/%v", url.QueryEscape(cmd.ClusterID))
	}
	logger := goa.NewLogger(log.New(os.Stderr, "", log.LstdFlags))
	ctx := goa.WithLogger(context.Background(), logger)
	resp, err := c.StatusCluster(ctx, path)
	if err != nil {
		goa.LogError(ctx, "failed", "err", err)
		return err
	}
...
func (cmd *StatusClusterCommand) RegisterFlags(cc *cobra.Command, c *client.Client) {
	...
	cc.Flags().StringVar(&cmd.ClusterID, "cluster_id", clusterID, `Cluster ID`)
}
